...
void cpp_renderer::Renderer::setResolution(unsigned int width, unsigned int height)
{
    width_ = width;
    height_ = height;
    depth_buffer_.clear(width_, height_, NAN);
    uv_buffer_.clear(width_, height_);
    poly_id_buffer_.clear(width_, height_);
}

cpp_renderer::RGBA32Image cpp_renderer::Renderer::render(const std::vector<Triangle> &polygons, const Camera &camera)
{
    ...
    for (auto x = 0; x < width; ++x)
    {
        ...
        {
            if (std::isnan(depth_buffer_(x, y)))
            { // no object at (x, y)
                continue;
            }
